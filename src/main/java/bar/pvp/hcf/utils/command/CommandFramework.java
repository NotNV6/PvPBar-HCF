package bar.pvp.hcf.utils.command;
...
public class CommandFramework implements CommandExecutor {
    ...
    private HCTeams plugin;

    public CommandFramework(HCTeams plugin) {
        this.plugin = plugin;
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && !sender.hasPermission(command.permission())) {
                    sender.sendMessage(plugin.getLangConfig().getString("ERROR.NO_PERMISSION"));
                    return true;
                }
                ...
                if (command.inGameOnly() && sender instanceof Player && command.inFactionOnly()) {
                    Profile profile = Profile.getByUuid(((Player) sender).getUniqueId());
                    PlayerTeam faction = profile.getFaction();
                    if (faction == null) {
                        sender.sendMessage(plugin.getLangConfig().getString("ERROR.NOT_IN_FACTION"));
                        return true;
                    }
                    if (command.isLeaderOnly() && !faction.getLeader().equals(profile.getUuid()) && !sender.hasPermission("ekko.admin")) {
                        sender.sendMessage(plugin.getLangConfig().getString("ERROR.NOT_LEADER"));
                        return true;
                    }
                    if (command.isOfficerOnly() && !faction.getLeader().equals(profile.getUuid()) && !faction.getOfficers().contains(profile.getUuid()) && !sender.hasPermission("ekko.admin")) {
                        sender.sendMessage(plugin.getLangConfig().getString("ERROR.NOT_OFFICER_OR_LEADER"));
                        return true;
                    }
                }
                try {
